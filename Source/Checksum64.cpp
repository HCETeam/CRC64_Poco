...
Poco::UInt64 Checksum64::initCRC64Val = 0xFFFFFFFFFFFFFFFFULL;
Poco::UInt64 Checksum64::finXORVal = 0xFFFFFFFFFFFFFFFFULL;
Poco::UInt64 Checksum64::crc_table[crc64_table_size] = {
...
Checksum64::Checksum64():
        _value(0)
{
// Here we lock access(...its calculating) to the static CRCTable. It need, if we use Checksum64 objects in separate threads.
}
...
void Checksum64::update(const char* data, unsigned int length)
{ 
  _value = initCRC64Val;
  unsigned long long       __crc0 = _value; 
  ...
  _value = __crc0 ^ finXORVal;
}

// Not used at time
void Checksum64::calcCrcTable()
{
/*  
  const unsigned long long CRCPOLY = 0x42F0E1EBA9EA3693ULL;
  register unsigned long long t = 0; 
  for(int i = 0; i < 256; i++) 
  {
    t = i;
    for(int j = 0; j < 8; j++) 
    {
      t = (t & 1) ? (CRCPOLY ^ (t >> 1)) : (t >> 1);
    }
    crc_table[i] = t;
  }
*/  
